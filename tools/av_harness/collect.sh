...
precheck() {
  ...
  local nc
  nc="$("$R" 'pgrep -c -f vmc-thinclient-ap[p] || true' 2>/dev/null | tr -d "[:space:]")"
  [ -z "$nc" ] && nc=0
  if [ "$nc" != "1" ]; then
    echo "PRECHECK: expected exactly 1 vmc-thinclient-ap[p], found $nc"
    fail=1   # >1 is the silent-sink trap; 0 means it died
  fi

  local ns
  ns="$(pgrep -c -f vmc-dash-si[m] 2>/dev/null || true)"; [ -z "$ns" ] && ns=0
  if [ "$ns" != "1" ]; then
    echo "PRECHECK: expected exactly 1 vmc-dash-si[m], found $ns"; fail=1
  fi

  local pt1 pt2
  pt1="$(curl -s --max-time 3 "$MPD_URL" | grep -o 'publishTime="[^"]*"' | head -1)"
  sleep 2
  pt2="$(curl -s --max-time 3 "$MPD_URL" | grep -o 'publishTime="[^"]*"' | head -1)"
  if [ -z "$pt1" ] || [ -z "$pt2" ]; then
    echo "PRECHECK: MPD did not parse at $MPD_URL"; fail=1
  elif [ "$pt1" = "$pt2" ]; then
    echo "PRECHECK: MPD publishTime not advancing ($pt1) - encoder stalled?"; fail=1
  fi
...
CPID="$("$R" 'pgrep -f vmc-thinclient-ap[p] | head -1' 2>/dev/null | tr -d '[:space:]')"
...
# ---- post-window EOS capture ----------------------------------------------
# The measured window ends 10 s before the clip does; the client emits its
# `eos` event during the drain tail, which must be present for the full-tier
# completion gate. Wait (up to EOS_WAIT_S) for it without disturbing the
# already-collected window, then capture the final byte offsets.
if [ "$TIER" = "full" ] && [ "$PLAY_ONCE" = "1" ]; then
  eos_wait_s=${EOS_WAIT_S:-90}
  i=0
  while [ "$i" -lt "$eos_wait_s" ]; do
    if "$R" --quiet "grep -q '\"t\":\"eos\"' $CLIENT_TLM 2>/dev/null"; then
      log "eos captured after ${i}s"
      break
    fi
    sleep 1
    i=$((i + 1))
