...
"$R" 'sudo pkill -9 -f vmc-thinclient-ap[p] || true' >/dev/null 2>&1
for i in $(seq 1 20); do
  n="$("$R" 'pgrep -c -f vmc-thinclient-ap[p] || true' 2>/dev/null | tr -d "[:space:]")"
  [ -z "$n" ] && n=0
  [ "$n" = "0" ] && break
  sleep 0.5
done
...
pkill -9 -f 'vmc-dash-si[m]' >/dev/null 2>&1 || true
pkill -9 -f 'ffmpeg.*das[h]' >/dev/null 2>&1 || true
for i in $(seq 1 20); do
  pgrep -f 'vmc-dash-si[m]' >/dev/null 2>&1 || break
  sleep 0.5
done
pgrep -f 'vmc-dash-si[m]' >/dev/null 2>&1 && { log "server still running after 10s"; exit 11; }
for i in $(seq 1 20); do
...
# Live when the MPD parses AND publishTime advances twice (proves the
# encoder is actually producing, not just that a file exists). The ffmpeg
# dash muxer grows the SegmentTimeline and keeps startNumber=1 until the
# rolling window fills, so startNumber is NOT a reliable liveness signal.
sn_prev=""; advances=0
for i in $(seq 1 60); do
  ...
  pt="$(printf '%s' "$mpd" | grep -o 'publishTime="[^"]*"' | head -1)"
  [ -z "$pt" ] && continue
  if [ -n "$sn_prev" ] && [ "$pt" != "$sn_prev" ]; then advances=$((advances+1)); fi
  sn_prev="$pt"
  [ "$advances" -ge 2 ] && break
done
[ "$advances" -ge 2 ] || { log "MPD never went live (advances=$advances)"; exit 12; }
log "server live (publishTime=$sn_prev)"

# ---------------------------------------------------------------- 5. start client
log "starting vmc-thinclient-ap[p]"
"$R" "setsid nohup env VMC_DRM=$VMC_DRM VMC_AUDIO_DEV=$VMC_AUDIO_DEV \
      ALSA_CONFIG_PATH=$VMC_ALSA_CONF VMC_TELEMETRY=$CLIENT_TLM \
      $CLIENT_BUILD/vmc-thinclient-ap[p] --dash $MPD_URL 1 \
